...
import { faLock, faCheckCircle, faShippingFast, faScroll } from "@fortawesome/free-solid-svg-icons";
      ...
      {data.map((media, index) => {
        return (
          <Row key={index} className={`mt-5 mb-5 ${props.mediaStyle || ''}`}>
            ...
            <Col>
              <Row>
                <p className="sub-header text-left font-weight-bold pb-4">{media.top}</p>
              </Row>
              <Row>
                <p className="sub-header text-left font-weight-bold">{media.bottom}</p>
              </Row>
            </Col>
          </Row>
        );
